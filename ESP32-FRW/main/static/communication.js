...
const DEFAULT_RESPONSE_TIMEOUT_MS = 2000; //ms
    ...
    if (Command.isValidQuery(cmd)) {
        if (params) { // Register commands
            if(cmd === Command.Query.FaceDescriptors){
                value = params;
            }else if(cmd === Command.Query.ConnectedNet || cmd === Command.Query.AvailableNets){
...
class LSS {
    ...
    connectCMDSocket() {
        ...
        this.cmdSK = new WebSocket(`ws://${this.gateway}/cmd`,"http");
    ...
    connectCAMSocket(imgElement) {
        ...
        this.cameraSK = new WebSocket(`ws://${this.gateway}/camera`,"http");
        this.setupCAMSocket();
    }
    ...
    configNetwork(ssid,password = ""){
        if(this.isCmdConnected() && ssid != null){
            let cmd = "#CNET@"+ssid+"@"+password+"\r";
            this.cmdSK.send(cmd);
            return 0;
        }else{
            return 1;
    ...
    genericQuery(command, register) {
        return new Promise(async (resolve, reject) => {
            ...
            const timeout = QUERY_TIMEOUT[command] !== undefined ? QUERY_TIMEOUT[command] : DEFAULT_RESPONSE_TIMEOUT_MS;
    ...
    // WIFI QUERY
    scanNetworks = async () => await this.genericQuery(Command.Query.AvailableNets);
    
    queryCurrentNetwork = async () => await this.genericQuery(Command.Query.ConnectedNet);
